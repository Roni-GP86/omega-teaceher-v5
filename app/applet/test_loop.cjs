...
["public/rpm_builder.html", "public/rpm_builder_b.html", "public/rpm_builder_c.html"].forEach(f => { 
            ...
            try {
                ...
                if (!res.ok) {
                    if (res.status === 400 || res.status === 401 || res.status === 403 || res.status === 404) {
                         ...
                         if (res.status === 404) throw new Error("FATAL: Model AI tidak ditemukan (kode 404). Kemungkinan versi API berubah.");
                         throw new Error("FATAL: Permintaan ditolak API (kode " + res.status + ")");
                    }
    ...
    if (startIndex > -1) {
       const nextFnIndex = html.indexOf("window.generateKarakteristikMateri =", startIndex);
       if (nextFnIndex > -1) {
