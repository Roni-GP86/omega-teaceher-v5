(() => {
  ...
  var MODELS = ["Problem Based Learning (PBL)", "Discovery Learning", "Inquiry Learning", "Project Based Learning (PjBL)", "Problem Solving", "Cooperative Learning"];
  var APPROACHES = ["Pembelajaran Mendalam (Deep Learning)", "Saintifik (Scientific)", "Kontekstual (CTL)", "Berorientasi pada Murid (Student-Centered)", "Diferensiasi (Differentiated Instruction)", "Konstruktivisme"];
  var KARAKTERISTIK_SISWA_LIST = [
    { title: "Berpikir Konkrit", icon: "\u{1F9E9}", desc: "Membutuhkan alat peraga nyata untuk memahami konsep belajar." },
    { title: "Rentang Konsentrasi Pendek", icon: "\u23F3", desc: "Fokus belajar terbatas sekitar 10-15 menit per aktivitas." },
    { title: "Suka Bergerak dan Bermain", icon: "\u{1F3C3}", desc: "Belajar lebih efektif melalui aktivitas fisik dan permainan." },
    { title: "Emosi Masih Labil", icon: "\u{1F3AD}", desc: "Membutuhkan bimbingan untuk mengelola perasaan dan interaksi." },
    { title: "Motorik Halus Belum Matang", icon: "\u270D\uFE0F", desc: "Sedang dalam proses melatih koordinasi tangan untuk menulis." },
    { title: "Motivasi Belajar Eksternal", icon: "\u{1F31F}", desc: "Semangat belajar dipicu oleh pujian, hadiah, atau dukungan guru." }
  ];
  var PROFIL_LULUSAN_DATA = [
    { title: "Keimanan dan Ketakwaan", icon: "\u{1F64F}", desc: "Berakhlak mulia kepada Tuhan YME dan sesama manusia." },
    { title: "Kewargaan", icon: "\u{1F1EE}\u{1F1E9}", desc: "Cinta tanah air dan sadar akan hak serta kewajiban negara." },
    { title: "Penalaran Kritis", icon: "\u{1F9E0}", desc: "Mampu menganalisis informasi secara objektif dan logis." },
    { title: "Kreativitas", icon: "\u{1F4A1}", desc: "Menghasilkan gagasan orisinal dan karya yang bermakna." },
    { title: "Kolaborasi", icon: "\u{1F91D}", desc: "Bekerja sama secara efektif untuk mencapai tujuan bersama." },
    { title: "Kemandirian", icon: "\u{1F464}", desc: "Bertanggung jawab penuh atas proses and hasil belajarnya." },
    { title: "Kesehatan", icon: "\u{1F4AA}", desc: "Menjaga keseimbangan jasmani dan rohani secara optimal." },
    { title: "Komunikasi", icon: "\u{1F91D}", desc: "Menyampaikan ide dan perasaan secara santun dan jelas." }
  ];
  var MITRA_INTERNAL_DATA = [
    { label: "Guru Kelas", icon: "\u{1F468}\u200D\u{1F3EB}", role: "perancang utama dan fasilitator pembelajaran" },
    { label: "Guru Mapel", icon: "\u{1F469}\u200D\u{1F52C}", role: "pemberi materi spesifik dan kolaborator instruksional" },
    { label: "Kepala Sekolah", icon: "\u{1F3EB}", role: "pembina kurikulum dan penyedia sarana prasarana" },
    { label: "Peserta Didik", icon: "\u{1F392}", role: "subjek aktif yang mengkonstruksi pengetahuan" },
    { label: "Penjaga Sekolah", icon: "\u{1F6E1}\uFE0F", role: "penjamin keamanan dan kenyamanan lingkungan belajar" },
    { label: "Tenaga Kebersihan", icon: "\u{1F9F9}", role: "penjaga kebersihan dan kesehatan lingkungan belajar" },
    { label: "Teman Sebaya", icon: "\u{1F91D}", role: "partner diskusi dan tutor sebaya dalam belajar" },
    { label: "Tendik", icon: "\u{1F3E2}", role: "pendukung administrasi proses pendidikan" },
    { label: "Perpustakaan", icon: "\u{1F4DA}", role: "penyedia referensi dan literasi pendukung materi" }
  ];
  var MITRA_EKSTERNAL_DATA = [
    { label: "Orang Tua", icon: "\u{1F468}\u200D\u{1F469}", role: "pendamping belajar di rumah dan partner pembentukan karakter" },
    { label: "Masyarakat", icon: "\u{1F3D8}\uFE0F", role: "sumber belajar kontekstual dan laboratorium sosial" },
    { label: "Puskesmas", icon: "\u{1F3E5}", role: "penyedia informasi kesehatan dan kebugaran murid" },
    { label: "Tokoh Masyarakat/Adat", icon: "\u{1F468}\u200D\u{1F4BC}", role: "narasumber kearifan lokal dan budaya daerah" },
    { label: "Perpustakaan Daerah", icon: "\u{1F3DB}\uFE0F", role: "perluasan akses literasi di luar sekolah" },
    { label: "Praktisi Ahli", icon: "\u{1F4CB}", role: "narasumber ahli yang memberikan wawasan dunia nyata" },
    { label: "Pengawas", icon: "\u{1F9D0}", role: "pendamping kualitas dan penjamin standar mutu pendidikan" },
    { label: "Aparat Desa", icon: "\u{1F3E2}", role: "pendukung keamanan dan integrasi sekolah dengan wilayah" },
    { label: "Narasumber", icon: "\u{1F5E3}\uFE0F", role: "pakar tamu yang memperkaya pemahaman materi" }
  ];
  var METHODS_WITH_DESC = [
    { label: "Metode Bermain", icon: "\u{1F3AE}" },
    { label: "Metode Demonstrasi", icon: "\u{1F441}\uFE0F" },
    { label: "Metode Cerita", icon: "\u{1F4D6}" },
    { label: "Metode Bernyanyi dan Bergerak", icon: "\u{1F3B6}" },
    { label: "Metode Praktik Langsung", icon: "\u{1F6E0}\uFE0F" },
    { label: "Tanya Jawab Sederhana", icon: "\u2753" }
  ];
  var BUDAYA_BELAJAR = [{ label: "Budaya Disiplin", icon: "\u23F1\uFE0F" }, { label: "Saling Menghargai", icon: "\u{1F64F}" }, { label: "Aktif Bertanya", icon: "\u{1F64B}\u200D\u2642\uFE0F" }, { label: "Kerja Sama", icon: "\u{1F465}" }, { label: "Refleksi Diri", icon: "\u{1F9D8}" }, { label: "Jujur & Tanggung Jawab", icon: "\u2705" }];
  var LINGKUNGAN_FISIK = [{ label: "Ruang Kelas", icon: "\u{1F3EB}" }, { label: "Halaman Sekolah", icon: "\u{1F333}" }, { label: "Lapangan Sekolah", icon: "\u26BD" }, { label: "Laboratorium", icon: "\u{1F9EA}" }, { label: "Ruang Seni", icon: "\u{1F3A8}" }, { label: "Kebun Sekolah", icon: "\u{1F33F}" }];
  var NON_DIGITAL_MEDIA = [{ label: "Buku Teks / LKS", icon: "\u{1F4DA}" }, { label: "Alat Peraga Konkret", icon: "\u{1F9F1}" }, { label: "Papan Tulis & Spidol", icon: "\u270D\uFE0F" }, { label: "Gambar / Poster Cetak", icon: "\u{1F5BC}\uFE0F" }, { label: "Flashcards / Kartu", icon: "\u{1F0CF}" }, { label: "Lingkungan Sekitar", icon: "\u{1F333}" }];
  var DIGITAL_PLATFORM = [{ label: "Quizizz / Kahoot", icon: "\u{1F3AE}" }, { label: "Google Workspace", icon: "\u2601\uFE0F" }, { label: "Canva Pendidikan", icon: "\u{1F3A8}" }, { label: "YouTube Eduasi", icon: "\u{1F4FA}" }, { label: "Zoom / G-Meet", icon: "\u{1F4F9}" }, { label: "WhatsApp Group", icon: "\u{1F4AC}" }];
  var DIGITAL_DEVICE = [{ label: "Laptop / Komputer", icon: "\u{1F4BB}" }, { label: "Smartphone / Tablet", icon: "\u{1F4F1}" }, { Pelabel: "LCD Proyektor", icon: "\u{1F4FD}\uFE0F" }, { label: "Speaker Aktif", icon: "\u{1F50A}" }, { label: "Jaringan Internet", icon: "\u{1F310}" }, { label: "Papan Tulis Digital", icon: "\u{1F58A}\uFE0F" }];
  var DIGITAL_MEDIA = [{ label: "Video Pembelajaran", icon: "\u{1F39E}\uFE0F" }, { label: "Slide Presentasi", icon: "\u{1F4FD}\uFE0F" }, { label: "E-Book / Modul PDF", icon: "\u{1F4C4}" }, { label: "Game Eduasi", icon: "\u{1F9E9}" }, { label: "Infografis Digital", icon: "\u{1F5BC}\uFE0F" }, { label: "Podcast / Audio", icon: "\u{1F399}\uFE0F" }];
  var currentModuleData = { custom_data: {} };
  var isProcessing = false;
  window.saveInputs = function() {
    const inputs = document.querySelectorAll('input[type="text"], textarea, select');
    inputs.forEach((i) => {
      if (i.id && i.id !== "cp_preview") currentModuleData[i.id] = i.value;
    });
  };
  ...
  function navigateTo(page) {
    ...
    } else if (page === "archive") currentStep = -2;
    else if (page === "contact") currentStep = -3;
    else currentStep = -1;
    render();
  }
  ...
  window.updateStep3Model = function(val) {
    ...
    if (sintaksArea) {
      if (val && SINTAKS_DATA[val]) {
        sintaksArea.innerHTML = `<div class="p-6 bg-slate-900 border-2 border-blue-900 rounded-3xl shadow-inner animate-fade-in"><h4 class="text-xs font-black uppercase mb-3 text-yellow-500">\u{1F4CC} Alur Sintak:</h4><div class="text-[10px] font-extrabold uppercase leading-relaxed text-blue-400">${SINTAKS_DATA[val].steps.join(" \u25B6 ")}</div></div>`;
      } else {
      ...
      if (val && !currentModuleData.pendekatan) {
        ...
        recArea.innerHTML = `
                    <div class="p-6 bg-blue-900/20 border-2 border-blue-900 rounded-3xl animate-fade-in shadow-sm">
                        <p class="text-[11px] uppercase font-black text-blue-400 mb-3 flex items-center gap-2"><span>\u{1F4A1}</span> Rekomendasi Pendekatan:</p>
      ...
      if (val) {
        ...
        recMetodeArea.innerHTML = `
                    <div class="p-6 bg-slate-900/50 border-2 border-blue-900 rounded-3xl animate-fade-in">
                        <p class="text-[11px] uppercase font-black text-blue-400 mb-3 flex items-center gap-2"><span>\u{1F6E0}\uFE0F</span> Rekomendasi Metode:</p>
            ...
            <div class="max-w-4xl w-full bg-black rounded-[3rem] shadow-2xl overflow-hidden border-4 border-yellow-500 flex flex-col animate-fade-in">
                <div class="pt-16 pb-12 px-8 md:px-12 bg-blue-900/30 text-center border-b-4 border-yellow-500 font-black">
                    <div class="flex items-center justify-center gap-4 whitespace-nowrap overflow-hidden">
                        <span class="text-4xl md:text-6xl shrink-0 font-black italic">\u{1F916}</span>
                ...
                <div class="p-8 md:p-12 text-center bg-transparent flex-1 font-black italic">
                    <div class="text-6xl mb-6 font-black text-[#FFD700]">\u{1F4D7}</div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                        ...
                        <button onclick="navigateTo('archive')" class="btn-primary py-4 px-8 rounded-2xl shadow-lg flex items-center justify-center gap-2 font-black italic uppercase !bg-slate-900 hover:!bg-slate-800 border-2 border-blue-900"><span>\u{1F4C2}</span> Arsip Modul</button>
                        <button onclick="window.open('https://youtu.be/7zNqKZTS8w4', '_blank')" class="bg-red-700 hover:bg-red-800 text-white font-bold py-4 px-8 rounded-2xl shadow-lg flex items-center justify-center gap-2 font-black italic uppercase border-2 border-yellow-500"><span>\u{1F4FA}</span> TUTORIAL</button>
                        <button onclick="navigateTo('contact')" class="btn-primary py-4 px-8 rounded-2xl shadow-lg flex items-center justify-center gap-2 font-black italic uppercase !bg-blue-950 hover:!bg-blue-900 border-2 border-blue-900"><span>\u{1F464}</span> CONTACT</button>
                    </div>
                </div>
            </div>
        </div>`;
        ...
        <div class="w-full h-full gradient-bg-primary overflow-auto flex items-center justify-center p-4 builder-ui">
            <div class="max-w-md w-full bg-black rounded-[3rem] shadow-2xl overflow-hidden border-4 border-yellow-500 animate-fade-in relative p-8 text-center flex flex-col items-center">
                ...
                <div class="flex flex-col w-full gap-3">
                    <button onclick="window.open('https://www.facebook.com/share/1DXzxmC4oN/', '_blank')" class="w-full btn-gold py-4 rounded-2xl shadow-lg uppercase italic flex items-center justify-center gap-2 tracking-tighter"><span>\u{1F310}</span> Hubungi</button>
            ...
            <div class="modal-content !w-[450px] max-h-[85vh] overflow-y-auto font-black italic !bg-black !text-white border-yellow-500">
                ...
                <div class="space-y-4 mb-8 text-white">
                    <div class="bg-blue-900/20 p-4 rounded-2xl border-2 border-blue-900">
                        <label class="block text-[10px] uppercase text-blue-400 mb-1">\u{1F4CD} Tempat & Tanggal Pembuatan</label>
                    ...
                    <div class="bg-blue-900/20 p-4 rounded-2xl border-2 border-blue-900">
                        <label class="block text-[10px] uppercase text-blue-400 mb-1">\u{1F464} Penyusun: <b class="text-yellow-500">Martha Fallo, S.Pd</b></label>
                    ...
                    <div class="bg-blue-900/20 p-4 rounded-2xl border-2 border-blue-900">
                        <label class="block text-[10px] uppercase text-blue-400 mb-1">\u{1F3EB} Kepala Sekolah</label>
                ...
                <div class="flex flex-col gap-2">
                    <button onclick="window.processKonfirmasi()" class="w-full btn-gold py-4 rounded-2xl shadow-lg uppercase italic tracking-tighter">\u{1F4BE} Simpan & Generate Modul</button>
        ...
        <div class="min-h-screen gradient-bg-primary p-6 md:p-12 builder-ui font-black italic animate-fade-in">
            <div class="max-w-5xl mx-auto">
                <div class="flex justify-between items-center mb-10">
                    <h2 class="text-4xl font-black text-yellow-500 uppercase">\u{1F4C2} Arsip Modul</h2>
      ...
      return `
                        <div class="bg-black p-6 rounded-[2rem] shadow-lg border-2 border-blue-900 flex flex-col justify-between italic relative overflow-hidden group">
                            ...
                            <div class="flex gap-2 mt-6 relative">
                                ...
                                <button onclick="window.deleteModule('${m.__backendId}')" class="bg-red-900/30 text-red-500 px-4 rounded-xl border-2 border-red-900/50 hover:bg-red-900/50 transition-colors italic">\u{1F5D1}\uFE0F</button>
                            </div>
                        </div>`;
    }).join("")}
                </div>`}
  ...
  window.openModule = function(id) {
    const modules = JSON.parse(localStorage.getItem("ronai_master_v50_db") || "[]");
    const found = modules.find((m) => m.__backendId === id);
    if (found) {
      currentModuleData = found;
      currentStep = 7;
      render();
      showToast("\u{1F4C2} Modul Berhasil Dibuka!");
    }
  };
  window.deleteModule = function(id) {
    let modules = JSON.parse(localStorage.getItem("ronai_master_v50_db") || "[]");
    modules = modules.filter((m) => m.__backendId !== id);
    localStorage.setItem("ronai_master_v50_db", JSON.stringify(modules));
    render();
    showToast("\u{1F5D1}\uFE0F Modul Dihapus!");
  };
  function renderCreate() {
    ...
    if (currentStep === 7) {
      document.getElementById("app").innerHTML = `<div class="w-full h-full bg-black overflow-auto p-4 md:p-8 ${animationClass}"><div class="max-w-[210mm] mx-auto mb-6 no-print flex flex-col items-center font-black italic"><div class="flex flex-wrap justify-center gap-4 items-center mb-4"><button onclick="prevStep()" class="bg-blue-900 text-white font-bold py-3 px-6 rounded-xl shadow uppercase text-xs font-black italic border-2 border-yellow-500">\u2B05\uFE0F Kembali Edit</button><button onclick="window.downloadWord()" class="bg-black text-yellow-500 font-bold py-3 px-6 rounded-xl shadow uppercase text-xs hover:bg-blue-900/20 italic border-2 border-blue-900">\u{1F4E5} Download Word</button><button onclick="window.downloadPDF()" class="btn-gold py-3 px-6 rounded-xl shadow uppercase text-xs hover:bg-yellow-600 italic border-2 border-blue-950">\u{1F4E5} Download PDF</button><button onclick="window.saveFinal()" class="bg-blue-600 text-white font-bold py-3 px-6 rounded-xl shadow uppercase text-xs font-black italic hover:bg-blue-700">\u{1F4BE} Simpan Data</button></div><p class="text-[10px] text-yellow-500 font-bold uppercase tracking-widest italic bg-blue-950 px-4 py-1 rounded-full border border-yellow-500">\u2728 Mode Edit Aktif: Klik teks di bawah untuk mengubah konten sebelum diunduh.</p></div><div id="capture-area" style="line-height: 1.5;">${content}</div></div>`;
    } else {
      document.getElementById("app").innerHTML = `
  ...
  function setAILoading(btn, loading) {
    if (loading) {
      if (btn) btn.dataset.oldText = btn.innerHTML;
      if (btn) btn.innerHTML = `<span class="loader"></span> Memproses...`;
      if (btn) btn.disabled = true;
      isProcessing = true;
    } else {
      if (btn) btn.innerHTML = btn.dataset.oldText;
  ...
  window.generateKarakteristikMateri = async function() {
    ...
    const d = currentModuleData;
    if (!d.selected_materi || d.selected_materi.length === 0) {
      showToast("\u26A0\uFE0F Pilih Materi Ajar di kiri!");
      return;
    }
  ...
  window.generateTP = async function() {
    ...
    const d = currentModuleData;
    if (!d.selected_materi || d.selected_materi.length === 0) {
      showToast("\u26A0\uFE0F Pilih materi di Langkah 2!");
      return;
    }
    setAILoading(btn, true);
    try {
  ...
  window.applyTPSelection = function() {
    ...
    if (el) el.value = finalTP;
    window.closeModal();
    showToast(`\u2728 TP Dipilih!`);
  };
  ...
  var aiGenericGen = async (key, context, btnElement) => {
    if (isProcessing) return;
    window.saveInputs();
    const d = currentModuleData;
...
3. Asesmen Sumatif
   ...
   (b). Cara/Sumber: [WAJIB disesuaikan dengan hasil nyata aktivitas peserta didik]
      ...
      if (key === "validasi_asesmen") {
        ...
        if (targetEl) {
          targetEl.innerText = results.soal;
          targetEl.classList.remove("text-slate-400");
          targetEl.classList.add("text-black");
        }
      } else {
        ...
        if (key === "validasi_rubrik") {
          ...
          if (rubEl) {
            rubEl.innerHTML = finalResult;
            rubEl.classList.remove("text-slate-400");
            rubEl.classList.add("text-black");
          }
        } else if (key === "validasi_lkpd") {
          const lkEl = document.getElementById("lkpd_preview");
          if (lkEl) {
            lkEl.innerHTML = finalResult;
            lkEl.classList.remove("text-slate-400");
            lkEl.classList.add("text-black");
          }
        } else {
          const targetEl = document.getElementById(key);
          if (targetEl) targetEl.value = finalResult;
        }
      }
  ...
  window.applyAsesmenSelection = function() {
    ...
    if (prevEl) {
      prevEl.innerHTML = finalAsesmen.trim();
      prevEl.classList.remove("text-slate-400");
      prevEl.classList.add("text-black");
    }
    const areaEl = document.getElementById("validasi_asesmen");
    if (areaEl) areaEl.value = finalAsesmen.trim();
    window.closeModal();
    showToast(`\u2728 Asesmen Utuh Dipilih!`);
  };
  ...
  window.generateLKPD = (e) => {
    if (!currentModuleData.validasi_asesmen) {
      showToast("\u26A0\uFE0F Silakan generate Asesmen terlebih dahulu!");
      return;
    }
    aiGenericGen("validasi_lkpd", "LKPD", e.currentTarget);
  };
  window.generateEvaluasi = (e) => {
    if (!currentModuleData.validasi_asesmen) {
      showToast("\u26A0\uFE0F Silakan generate Asesmen terlebih dahulu!");
      return;
    }
    aiGenericGen("validasi_evaluasi", "Evaluasi", e.currentTarget);
  };
  function stepInformasiUmum() {
    ...
    return `<div class="grid grid-cols-1 md:grid-cols-2 gap-6 font-black italic">
        <div class="col-span-1 md:col-span-2 bg-blue-950/20 p-5 rounded-3xl shadow-sm animate-fade-in border-l-8 border-blue-900">
            <label class="block text-xs font-black text-blue-400 uppercase mb-2">\u{1F3F7}\uFE0F Judul Modul</label>
        ...
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black uppercase text-blue-400 mb-1">\u{1F464} Penyusun</label><input type="text" id="penyusun" class="input-modern p-4 rounded-2xl !bg-slate-900 font-bold" value="Martha Fallo, S.Pd" readonly></div>
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black uppercase text-blue-400 mb-1">\u{1F3EB} Nama Sekolah</label><input type="text" id="sekolah" class="input-modern p-4 rounded-2xl !bg-slate-900 font-bold" value="SD Negeri Nifuboke" readonly></div>
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-yellow-500 animate-fade-in"><label class="block text-[10px] font-black uppercase text-blue-400 mb-1">\u{1F4C5} Tahun Pelajaran & Semester</label>
        ...
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black text-blue-400 uppercase mb-1">\u{1F3AF} Fase / Kelas</label><select id="fase_kelas" class="select-modern p-4 rounded-2xl font-bold"><option value="Fase A / Kelas 1" ${d.fase_kelas === "Fase A / Kelas 1" ? "selected" : ""}>Fase A / Kelas 1</option><option value="Fase A / Kelas 2" ${d.fase_kelas === "Fase A / Kelas 2" ? "selected" : ""}>Fase A / Kelas 2</option></select></div>
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black text-blue-400 uppercase mb-1">\u{1F4DA} Mata Pelajaran</label><select id="mapel" class="select-modern p-4 rounded-2xl font-bold" onchange="window.updateStep0()"><option value="">-- Pilih --</option>${Object.keys(CP_DATABASE_FASE_C).map((m) => `<option value="${m}" ${selectedMapel === m ? "selected" : ""}>${m}</option>`).join("")}</select></div>
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black text-blue-400 uppercase mb-1">\u{1F9E9} Elemen</label><select id="elemen" class="select-modern p-4 rounded-2xl font-bold" onchange="window.updateStep0()" ${!selectedMapel ? "disabled" : ""}> <option value="">-- Pilih --</option> ${elemenData ? Object.keys(elemenData).map((e) => `<option value="${e}" ${selectedElemen === e ? "selected" : ""}>${e}</option>`).join("") : ""}</select></div>
        <div class="bg-blue-950/20 p-5 rounded-3xl border-l-8 border-blue-900 animate-fade-in"><label class="block text-[10px] font-black text-blue-400 uppercase mb-1">\u{1F4D6} Topik Materi</label><select id="materi" class="select-modern p-4 rounded-2xl font-bold" onchange="window.saveInputs()" ${!selectedElemen ? "disabled" : ""}> <option value="">-- Pilih --</option> ${topikList ? topikList.map((t) => `<option value="${t.judul}" ${d.materi === t.judul ? "selected" : ""}>${t.judul}</option>`).join("") : ""}</select></div>
    ...
    return `<div class="space-y-8 font-black italic animate-fade-in text-white">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="bg-black p-6 border-2 rounded-[2.5rem] border-blue-900 flex flex-col shadow-inner">
                    <h3 class="text-xl font-black mb-6 text-center text-yellow-500 uppercase">\u{1F4D6} Materi Ajar</h3>
                ...
                <div class="md:col-span-2 bg-black border-2 border-yellow-500 p-6 rounded-[2.5rem] shadow-xl">
                    <div class="flex justify-between items-center mb-6 font-black italic"><h3 class="text-xl font-black text-yellow-500 uppercase">\u{1F50D} Karakteristik Materi</h3><button onclick="window.generateKarakteristikMateri()" class="btn-gold px-6 py-2 rounded-xl text-[10px] uppercase shadow-lg border border-blue-900">RonAI Analyze</button></div>
            ...
            <div class="bg-black p-8 border-2 rounded-[2.5rem] border-blue-900 shadow-sm">
                <h3 class="text-xl font-black text-center text-yellow-500 uppercase mb-8 italic">\u{1F466} Karakteristik Murid Fase A</h3>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 font-black italic">
                    ${fullKarakteristik.map((k, i) => {
      const isS = (d.selected_karakteristik || []).includes(k.title);
      return `<label class="flex flex-col gap-1 p-5 rounded-2xl border-2 transition-all ${isS ? "border-yellow-500 bg-blue-900 shadow-md scale-105" : "border-blue-900 bg-slate-900"} cursor-pointer hover:border-yellow-500"><div class="flex items-center gap-3"><input type="checkbox" onchange="window.toggleKarakteristik('${k.title}')" ${isS ? "checked" : ""} class="w-5 h-5 !accent-yellow-500"> <span class="text-xl">${k.icon}</span><span class="text-[10px] uppercase text-white font-black">${k.title}</span></div>${k.desc ? `<p class="text-[8px] text-slate-300 leading-tight mt-2 italic">${k.desc}</p>` : ""}</label>`;
    }).join("")}
                </div>
            ...
            <div class="bg-black p-8 border-4 border-blue-900 rounded-[2.5rem] shadow-2xl">
                <h3 class="text-xl font-black text-center text-yellow-500 uppercase mb-4 italic">\u{1F393} Dimensi Profil Lulusan</h3>
  ...
  function stepDesain() {
    ...
    return `<div class="space-y-10 font-black italic animate-fade-in text-white">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        ...
                        <div id="sintaks-area-container" class="space-y-4">
                            ${d.model ? `<div class="p-6 bg-slate-900 border-2 border-blue-900 rounded-3xl shadow-inner animate-fade-in"><h4 class="text-xs font-black uppercase mb-3 text-yellow-500">\u{1F4CC} Alur Sintak:</h4><div class="text-[10px] font-extrabold uppercase leading-relaxed text-blue-400">${SINTAKS_DATA[d.model].steps.join(" \u25B6 ")}</div></div>` : ""}
                        </div>
                    </div>
                    <div class="space-y-6">
            ...
            <div class="bg-black p-8 border-2 border-blue-900 rounded-[2.5rem] shadow-sm">
                <h3 class="text-xl font-black uppercase mb-8 text-center text-yellow-500 tracking-widest">\u{1F6E0}\uFE0F Metode Pembelajaran (Maks. 4)</h3>
    ...
    return `<div class="bg-black p-8 border-2 rounded-[3rem] shadow-sm font-black italic transition-all ${isSectionLocked ? "opacity-30 pointer-events-none grayscale bg-slate-950" : ""}" style="border-color: #1e3a8a">
      ...
      return `<label data-key="${key}" data-val="${item.label || item.title}" class="flex items-center gap-4 p-5 rounded-2xl border-2 transition-all ${isS ? "bg-blue-900 border-yellow-500 shadow-md scale-105" : "bg-slate-900 border-blue-900"} cursor-pointer hover:border-yellow-500">
                        ...
                        <div class="flex flex-col">
                            <span class="text-2xl">${item.icon || "\u{1F539}"}</span>
  ...
  window.toggleArr = function(key, v) {
    ...
    if (idx === -1) {
      if (isDigitalKey) {
        ...
        if (hasNonDigital) {
          showToast("\u26A0\uFE0F Hapus Media Nondigital dahulu!");
          return;
        }
      } else if (isNonDigitalKey) {
        const hasDigital = digitalKeys.some((k) => (currentModuleData[k] || []).length > 0);
        if (hasDigital) {
          showToast("\u26A0\uFE0F Hapus Media Digital/Aplikasi dahulu!");
          return;
        }
      }
    }
  ...
  function stepMitra() {
    return `<div class="space-y-10 font-black italic">${window.renderGridSection("\u{1F91D} Mitra Internal", "mitra_internal", MITRA_INTERNAL_DATA)}${window.renderGridSection("\u{1F30D} Mitra Eksternal", "mitra_eksternal", MITRA_EKSTERNAL_DATA)}</div>`;
  }
  function stepLingkungan() {
    return `<div class="space-y-10 font-black italic">${window.renderGridSection("\u{1F3D8}\uFE0F Lingkungan Fisik", "lingkungan_fisik", LINGKUNGAN_FISIK)}${window.renderGridSection("\u{1F64C} Budaya Belajar", "budaya_belajar", BUDAYA_BELAJAR)}</div>`;
  }
  function stepDigital() {
    return `<div class="space-y-12 font-black italic pb-10">${window.renderGridSection("\u{1F4BB} Aplikasi / Platform", "platform_aplikasi", DIGITAL_PLATFORM)}${window.renderGridSection("\u{1F4F1} Perangkat Digital", "perangkat_digital", DIGITAL_DEVICE)}${window.renderGridSection("\u{1F3AC} Media Digital", "media_digital", DIGITAL_MEDIA)}${window.renderGridSection("\u{1F4E6} Media Nondigital", "media_non_digital", NON_DIGITAL_MEDIA)}</div>`;
  }
  function stepValidasiAktivitas() {
      ...
      if (d.validasi_langkah) {
        ...
        previewHTML = `<div class="space-y-8"> 
          ...
          return `<div class="mb-6 group">
                            ...
                            ${s.prinsip_deep ? `<div class="mt-2 p-3 bg-blue-950 rounded-xl text-[10px] text-blue-300 border-l-4 border-blue-400">\u{1F48E} ${s.prinsip_deep}</div>` : ""}
                        </div>`;
        }).join("")}</div></div> 
    ...
    return `<div class="space-y-12 pb-10 font-black italic text-white">
            <div class="bg-black border-2 border-blue-900 p-8 rounded-[3rem] shadow-sm animate-fade-in"><div class="flex justify-between items-center mb-6"><label class="text-xl uppercase text-yellow-500 tracking-tighter">\u{1F3AF} Tujuan Pembelajaran</label><button onclick="window.generateTP()" class="btn-gold px-8 py-2 rounded-xl text-[10px] uppercase shadow-md">RonAI Formulate</button></div><textarea id="tp" rows="4" class="input-modern p-5 bg-slate-950 font-bold text-sm italic rounded-2xl border-none shadow-inner" style="line-height: 1.6;">${d.tp || ""}</textarea></div>
            <div class="bg-black p-8 rounded-[3rem] shadow-2xl border-4 border-blue-900"><div class="flex justify-between items-center mb-8"><h3 class="text-xl uppercase text-yellow-500 tracking-tighter">\u{1F4DD} Langkah Pembelajaran</h3><button onclick="window.generateLangkahPembelajaran()" class="btn-gold px-8 py-2 rounded-xl text-[10px] uppercase shadow-lg border border-blue-900">RonAI Generate</button></div><div class="max-h-[600px] overflow-y-auto p-2" style="line-height: 1.5;">${previewHTML}</div></div>
            ...
            <div class="flex flex-col gap-10">
                ...
                <div class="bg-black p-8 rounded-[3rem] border-2 border-blue-900 shadow-sm">
                    <div class="flex justify-between items-center mb-6"><h4 class="uppercase text-xs text-yellow-500 font-black tracking-widest">\u{1F4CA} Asesmen Pembelajaran</h4><button onclick="window.generateAsesmen(event)" class="btn-gold px-6 py-2 rounded-xl text-[10px] uppercase">Generate</button></div>
                ...
                <!-- LKPD (Warna Kertas) -->
                <div class="bg-black p-8 rounded-[3rem] border-2 border-blue-900 shadow-sm">
                    <div class="flex justify-between items-center mb-6"><h4 class="uppercase text-xs text-yellow-500 font-black tracking-widest">\u{1F4D1} Lembar Kerja Peserta Didik</h4><button onclick="window.generateLKPD(event)" class="btn-gold px-6 py-2 rounded-xl text-[10px] uppercase">Generate</button></div>
                ...
                <div class="bg-black p-8 rounded-[3rem] border-2 border-blue-900 shadow-sm">
                    <div class="flex justify-between items-center mb-6"><h4 class="uppercase text-xs text-yellow-500 font-black tracking-widest">\u{1F4DD} Soal Evaluasi Akhir</h4><button onclick="window.generateEvaluasi(event)" class="btn-gold px-6 py-2 rounded-xl text-[10px] uppercase">Generate</button></div>
  ...
  function stepReview() {
    ...
    const budayaMerged = (d.budaya_belajar || []).length > 0 ? `Pada proses pembelajaran ini, budaya belajar yang harus dihadirkan adalah ${d.budaya_belajar.join(", ")} yang melatih peserta didik untuk mengembangkan karakter positif, integritas, dan kedisiplinan dalam setiap tahap aktivitas pembelajaran.` : "-";
    ...
    try {
      ...
      evaluasiSoalHtml = d.validasi_evaluasi ? d.validasi_evaluasi.replace(/\\n/g, "<br>") : "-";
    }
    const combinedHtml = `
          ...
          return `
            ...
            return `<tr>
                                            ...
                                            <td style="text-align:justify;">
                                                ...
                                                ${s.prinsip_deep ? `<div style="font-size: 10pt; background-color: #f0f9ff; border: 1px solid #bae6fd; padding: 5px; border-radius: 4px; margin-top: 3px;">\u{1F48E} <b>${s.prinsip_deep}</b></div>` : ""}
                                            </td>
  ...
  window.toggleKarakteristik = function(v) {
    window.saveInputs();
    if (!currentModuleData.selected_karakteristik) currentModuleData.selected_karakteristik = [];
    const idx = currentModuleData.selected_karakteristik.indexOf(v);
    if (idx > -1) currentModuleData.selected_karakteristik.splice(idx, 1);
    else {
      if (currentModuleData.selected_karakteristik.length < 4) currentModuleData.selected_karakteristik.push(v);
      else showToast("\u26A0\uFE0F Maksimal 4!");
    }
    render();
  };
  window.toggleMateri = function(v) {
    window.saveInputs();
    if (!currentModuleData.selected_materi) currentModuleData.selected_materi = [];
    const idx = currentModuleData.selected_materi.indexOf(v);
    if (idx > -1) currentModuleData.selected_materi.splice(idx, 1);
    else {
      if (currentModuleData.selected_materi.length < 2) currentModuleData.selected_materi.push(v);
      else showToast("\u26A0\uFE0F Maksimal 2!");
    }
    render();
  };
  ...
  window.downloadPDF = function() {
    ...
    html2pdf().set({
      margin: [10, 0, 15, 0],
      filename: `Modul_Ajar_${currentModuleData.materi || "Generated"}.pdf`,
      html2canvas: { scale: 3 },
  ...
  window.downloadWord = function() {
    ...
    link.href = url;
    link.download = `${currentModuleData.judul_modul || "Modul_Ajar"}.doc`;
    link.click();
  };
  window.saveFinal = async function() {
    await dataSdk.create(currentModuleData);
    showToast("\u{1F4BE} Simpan di Arsip!");
  };
